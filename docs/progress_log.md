...
- `run.py` 新增 `--mode pd`、`--target-deg`、`--kp`、`--kd`、`--plot`、`--compare`；原 `--mode torque` 行为保持不变。
- 无窗口模式记录每个 `0.002 s` 物理步，生成角度/速度/力矩三联图；可额外比较弱 Kp、低/平衡/高 Kd。
- Viewer 模式通过 MuJoCo control callback 在每个物理步执行 PD；中文面板自动切到目标角度、Kp、Kd 精确输入，并展示位置误差、raw torque 和限幅状态。

### 验证结果

默认 Phase 2 参数为目标 `30°`、`Kp=30`、`Kd=3`、力矩范围 `[-2, 2] N·m`。3 秒仿真最终约为 `27.84°`，速度接近 `0`，误差约 `2.16°`；初期正确触发正向 2 N·m 限幅，随后退出饱和并用约 1.13 N·m 抵抗重力。响应图和四组增益对比图均成功生成。

新增测试覆盖公式、P/D 分项、限幅、参数校验、在线调参、PD 面板 API、CLI 运行与 PNG 产物；完整测试通过。纯 PD 在重力下存在静态误差，已作为 Phase 2 的学习结论记录，而不是提前加入重力补偿隐藏该现象。
