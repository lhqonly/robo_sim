...
验收：无图形界面也能运行仿真；打印或保存位置、速度和控制力矩；对应概念写入 `docs/concepts.md`。

完成结果（2026-07-10）：模型包含固定基座、单自由度 hinge、1 kg 杆件和力矩范围 `[-2, 2] N·m` 的 motor；恒力矩 CLI 会按时间打印 `position`、`velocity` 和 `torque`，模型结构/运动与 CLI 均有自动化测试。
