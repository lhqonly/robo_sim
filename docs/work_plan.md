...
| Phase 2 | 已完成 | 理解反馈与 PD 控制 | 通用 PD 控制器、响应曲线和参数对比 |
...
完成结果（2026-07-10）：`PDController` 每个物理时间步根据位置/速度误差计算力矩并按执行器范围限幅，支持无窗口数值验证、三联响应曲线、增益对比图和原生 Viewer。中文学习面板在 PD 模式下可精确修改目标角度、`Kp`、`Kd`，并实时显示误差、P/D 分项、目标保持力矩、当前位置重力/偏置矩与饱和状态；页面直接绘制最近 30 秒的目标/实际角度、速度和 raw/applied 力矩曲线。Phase 1 的恒扭矩模式保持兼容。
